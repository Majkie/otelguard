...
func InitializeApplication(cfg *config.Config) (*wire.Application, error) {
	...
	metricsHandler := wire.ProvideMetricsHandler(metricsService, logger)
	dashboardRepository := wire.ProvideDashboardRepository(pool)
	dashboardService := wire.ProvideDashboardService(dashboardRepository, logger)
