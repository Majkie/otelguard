...
func InitializeApplication(cfg *config.Config) (*wire.Application, error) {
	...
	validatorService := wire.ProvideValidatorService(logger)
	remediationService := wire.ProvideRemediationService(logger)
	guardrailService := wire.ProvideGuardrailService(guardrailRepository, guardrailEventRepository, validatorService, remediationService, logger)
	guardrailHandler := wire.ProvideGuardrailHandler(guardrailService, logger)
	guardrailAnalyticsService := wire.ProvideGuardrailAnalyticsService(guardrailRepository, guardrailEventRepository, logger)
	guardrailAnalyticsHandler := wire.ProvideGuardrailAnalyticsHandler(guardrailAnalyticsService, logger)
	...
	handlers := wire.ProvideHandlers(healthHandler, authHandler, orgHandler, traceHandler, otlpHandler, promptHandler, guardrailHandler, guardrailAnalyticsHandler, annotationHandler, feedbackHandler, llmHandler, agentHandler, evaluatorHandler, datasetHandler, experimentHandler, scoreAnalyticsHandler)
